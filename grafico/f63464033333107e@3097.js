...
function _data() {
  ...
  localStorage.setItem('dados288', JSON.stringify(data));
  console.log(data);
  


  const dadosConvertidos = data.map(obj => ({
    ...obj,
    category: parseInt(obj.category, 10),
    value: parseInt(obj.value, 10),
  }));

  function convertStringToDate(dataArray) {
    const newData = dataArray.map(item => {
      const newItem = { ...item }; // Criar um novo objeto para não modificar o original
      const dateString = newItem.date; // Obter a string de data
      const [year, month, day] = dateString.split('-'); // Separar a string em ano, mês e dia

      // Criar um novo objeto de data a partir dos componentes da data
      newItem.date = new Date(parseInt(year, 10), parseInt(month, 10) - 1, parseInt(day, 10));
      return newItem;
    });

    return newData;
  }
  const segundoc = convertStringToDate(dadosConvertidos);
  console.log(segundoc);
  return segundoc;
}
...
async function* _chart(replay, d3, width, height, bars, axis, labels, ticker, keyframes, duration, x, invalidation) {
  ...
  d3.select("#container")
    .node() // obtém o nó DOM
    .append(svg.node()); // adiciona o nó SVG ao DOM\
